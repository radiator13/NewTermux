...
object Logger {
     ...
     * prefix/suffix max 68 characters for itself.
     */
    ...
    fun getStackTracesStringArray(throwable: Throwable?): Array<String?>? {
        return getStackTracesStringArray(listOfNotNull(throwable))
    }
    ...
    fun getSingleLineLogStringEntry(label: String, `object`: Any?, def: String): String {
        return if (`object` != null) "$label: `${`object`}`" else "$label: $def"
    }
    ...
    @JvmStatic
    fun getLogLevelLabelsArray(context: Context?, logLevels: Array<CharSequence>?, addDefaultTag: Boolean): Array<String>? {
        if (logLevels == null) return null

        val logLevelLabels = arrayOfNulls<String>(logLevels.size)
        ...
        return logLevelLabels as Array<String>
    }
    ...
    /**
     * Check if custom log level is valid and >= [CURRENT_LOG_LEVEL]. If custom log level is
     * not valid then [LOG_LEVEL_VERBOSE] must be >= [CURRENT_LOG_LEVEL].
     */
    @JvmStatic
    fun shouldEnableLoggingForCustomLogLevel(customLogLevel: Int?): Boolean {
        ...
        val effectiveLogLevel = if (isLogLevelValid(customLogLevel)) customLogLevel else LOG_LEVEL_VERBOSE
        return effectiveLogLevel >= CURRENT_LOG_LEVEL
    }
}
