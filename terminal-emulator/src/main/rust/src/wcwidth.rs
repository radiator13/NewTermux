/// Port of WcWidth.kt - Implementation of wcwidth(3) for Unicode 15.
///
...
static ZERO_WIDTH: &[(u32, u32)] = &[
    ...
    (0x180f, 0x180f),  // Mongolian Free Variation..Mongolian Free Variation
    ...
    (0x1ab0, 0x1ace),  // Combining Doubled Circum..Combining Latin Small Le
    ...
    (0xa82c, 0xa82c),  // Syloti Nagri Sign Altern..Syloti Nagri Sign Altern
...
static WIDE_EASTASIAN: &[(u32, u32)] = &[
    ...
    (0x3250, 0x4dbf),  // Partnership Sign        ..Cjk Unified Ideograph-4d
...
/// Binary search to check if a code point is in a lookup table.
fn intable(table: &[(u32, u32)], c: u32) -> bool {
...
pub fn width(ucs: u32) -> i32 {
    // Fast path for ASCII range (0-127) - avoids binary search for 95%+ of characters
    if ucs < 128 {
        // Control chars (0-31), DEL (127), and C1 control start (0x7F) are zero width
        // Printable ASCII (32-126) is width 1
        if ucs < 32 || ucs == 127 {
            return 0;
        } else {
            return 1;
...
/// The width at an index position in a u16 char array (UTF-16).
/// Handles surrogate pairs: if the char at index is a high surrogate,
/// combines it with the next char (low surrogate) to form a full code point.
pub fn width_from_chars(chars: &[u16], index: usize) -> i32 {
    let c = chars[index];
    if is_high_surrogate(c) {
        // Combine high and low surrogates to form a full code point
        let code_point = to_code_point(c, chars[index + 1]);
        width(code_point)
    } else {
        width(c as u32)
    }
...
/// Count the number of zero-width characters (combining characters) in the
/// `chars` array from `start` index to `end` index (exclusive).
pub fn zero_width_count(chars: &[u16], start: usize, end: usize) -> i32 {
    if start >= chars.len() {
        return 0;
    }
    ...
    while i < end && i < chars.len() {
        if is_high_surrogate(chars[i]) {
            if i + 1 < chars.len() {
                let code_point = to_code_point(chars[i], chars[i + 1]);
                if width(code_point) <= 0 {
                    count += 1;
                }
            }
            i += 2;
        } else {
            if width(chars[i] as u32) <= 0 {
                count += 1;
            }
            i += 1;
        }
...
/// Check if a u16 value is a UTF-16 high surrogate (0xD800-0xDBFF).
#[inline]
fn is_high_surrogate(c: u16) -> bool {
    (c & 0xFC00) == 0xD800
}

/// Combine a high and low surrogate pair into a full Unicode code point.
#[inline]
...
mod tests {
    ...
    #[test]
    fn test_ascii() {
        // Control chars are zero width
        assert_eq!(width(0), 0);
        assert_eq!(width(31), 0);
        assert_eq!(width(127), 0);
        // Printable ASCII is width 1
        assert_eq!(width(32), 1);
        assert_eq!(width(65), 1);
        assert_eq!(width(126), 1);
    }

    #[test]
    fn test_wide_chars() {
        // CJK characters should be width 2
        assert_eq!(width(0x4e00), 2); // 一
        assert_eq!(width(0x4e2d), 2); // 中
        assert_eq!(width(0x3041), 2); // あ
    }

    #[test]
    fn test_zero_width() {
        // Zero-width characters
        assert_eq!(width(0x0300), 0); // Combining Grave Accent
        assert_eq!(width(0x200B), 0); // Zero Width Space
        assert_eq!(width(0xfe00), 0); // Variation Selector-1
    }

    #[test]
    fn test_surrogate_pairs() {
        // Test emoji with surrogate pair (U+1F600 = 😀)
        let chars = vec![0xD83D, 0xDE00];
        assert_eq!(width_from_chars(&chars, 0), 2);
    }
}
